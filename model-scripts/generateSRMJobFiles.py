...
def write_eb_risk_config(regionPair):
    ...
    configContents = configTemplate.format(**{'region':region,'province':province})
    configFile = open('ebRisk_SRMJobFiles_output/ebRisk_b0_EP_{region}.ini'.format(**{'region':region}), 'w')
    configFile.write(configContents)
    configFile.close()
    # configTemplate.close()
    ...
    configContents = configTemplate.format(**{'region':region,'province':province})
    configFile = open('ebRisk_SRMJobFiles_output/ebRisk_r1_EP_{region}.ini'.format(**{'region':region}), 'w')
    configFile.write(configContents)
    configFile.close()
    # configTemplate.close()
    return
